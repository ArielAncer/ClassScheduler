...
function runScraper(school) {
	...
	var courses = casper.evaluate(function() {
		...
		function getTimeObject(obj, days) {
	      	return { 'start': $(obj).children('td:nth-child(7)').text().replace(" ", "").toLowerCase() || '', 
	   	    		 'end': $(obj).children('td:nth-child(8)').text().replace(" ", "").toLowerCase() || '',
	   	    		 'day': parseDays(days)
	   	    		 };
		...
		$('#collapseFour tr:not(:first-child)').each(function() {
		   	if ($(this).children('td').length === 1) {
		   		...
		   		if (!(/^[0-9]{1}$/.test(credits))) {
		   			...
		   			if (/^[\- a-zA-Z&\s\u00A0]{4,}$/.test($(this).children('td:nth-child(4)').text())) {
		   				days = $(this).children('td:nth-child(4)').text();
		   			}
		   		}
		   		else {
...
var fs = require('fs');
casper.start('http://las.touro.edu/men/course-schedule/', function() {
});
casper.then(function() {
	runScraper('Men');
});
